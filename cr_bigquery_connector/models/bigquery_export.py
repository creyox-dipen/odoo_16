...
class BigQueryExport(models.Model):
    ...
    cr_export_model_ids = fields.Many2many(
        ...
        help="Select the model to export to BigQuery",
        required=True,
    )
    cr_export_cron_value_ = fields.Integer(
        string="Schedule Export time", readonly=False, store=True
    )
    ...
    cr_import_model_ids = fields.Many2many(
        ...
        help="Select the model to import from BigQuery",
        required=True,
    )
    cr_import_cron_value_ = fields.Integer(
        string="Schedule Import time", readonly=False, store=True
    )
    ...
    cr_export_cron_job_id = fields.Many2one(
        "ir.cron", string="Scheduled Action", readonly=True
    )
    cr_import_cron_job_id = fields.Many2one(
        "ir.cron", string="Scheduled Action", readonly=True
    )
    ...
    _sql_constraints = [
        (
            "unique_cr_config_id",
            "UNIQUE(cr_config_id)",
            "This configuration is already used.",
        )
    ]
    ...
    def _compute_available_fields(self):
        for record in self:
            if record.cr_model_id:
                fields_domain = [
                    ("model", "=", record.cr_model_id.model),
                    ("store", "=", True),
                ]
    ...
    def _check_stored_fields(self):
        ...
        for record in self:
            if record.cr_model_columns:
                non_stored_fields = record.cr_model_columns.filtered(
                    lambda f: not f.store
                )
                if non_stored_fields:
                    raise ValidationError(
                        _(
                            "The following fields are not stored and cannot be exported: %s"
                        )
    ...
    def _get_bigquery_client(self, config):
        ...
        try:
            ...
            client = bigquery.Client(
                credentials=credentials, project=config.cr_project_id
            )
            return client
        except Exception as e:
            raise ValidationError(
                _("Failed to initialize BigQuery client: %s") % str(e)
            )
    ...
    def action_export_data(self, id):
        ...
        for model_record in record.cr_export_model_ids:
            ...
            query = (
                f"SELECT {', '.join(query_fields)} FROM {table} WHERE {where_clause}"
            )
    ...
    def action_import_data(self, id):
        ...
        for model in record.cr_import_model_ids:
            ...
            if not self.env["cr.big.query.scheduler"]._check_table_exists(
                client, table_id
            ):
                raise UserError(
                    _("The selected table '%s' does not exist in BigQuery.")
                    % model_name
                )
            ...
            for row in rows:
                ...
                if record_id is None:
                    ...
                    existing_record = self.env[model_name].search(
                        [(unique_identifier, "=", record_id)], limit=1
                    )
                    ...
                    for field_name in query_fields:
                        ...
                        if self.env["cr.big.query.scheduler"]._is_value_valid(
                            field_value
                        ):
                            if field.ttype == "many2one":
                                ...
                                if isinstance(field_value, list):
                                    valid_ids = [
                                        v for v in field_value if isinstance(v, int)
                                    ]
                                    if valid_ids:
                                ...
                                try:
                                    record_data[field_name] = fields.Date.from_string(
                                        field_value
                                    )
                                except ValueError:
                                ...
                                try:
                                    dt_value = (
                                        fields.Datetime.from_string(field_value)
                                    ...
                                    try:
                                        if isinstance(field_value, str):
                                            ...
                                            decoded_value = base64.b64decode(
                                                field_value, validate=True
                                            )
                                            record_data[field_name] = base64.b64encode(
                                                decoded_value
                                            ).decode("utf-8")
                                ...
                                record_data[field_name] = (
                                    float(field_value) if field_value else 0.0
                                )
                    ...
                    if account_code:
                        ...
                        if (
                            existing_account
                            and existing_account.id != existing_record.id
                        ):
                            continue
        ...
        for rec in self:
            vals = {
                ...
                "model_id": self.env.ref(
                    "cr_bigquery_connector.model_cr_big_query_export"
                ).id,
                "state": "code",
    ...
    def action_update_scheduler(self):
        for rec in self:
            if not rec.cr_export_cron_job_id:
                raise ValidationError(
                    _("No existing scheduler to edit. Please create one first.")
                )
            vals = {
                "interval_number": self.cr_export_cron_value_,
                "interval_type": self.cr_export_scheduled_units_,
