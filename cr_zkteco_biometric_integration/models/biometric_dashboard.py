...
class BiometricDashboard(models.AbstractModel):
    ...
    def get_dashboard_data(self):
        ...
        # 1. Total Employees
        total_employees = self.env['hr.employee'].search_count([('active', '=', True), ('company_id', '=', company.id)])
        ...
        present_ids = [res['employee_id'][0] for res in present_employee_ids if res['employee_id']]
        ...
        recent_punches = []
        logs = self.env['biometric.attendance.log'].search([], order='timestamp desc', limit=10)
        for log in logs:
            ...
            recent_punches.append({
                ...
                'type': 'Check In' if log.verify_state in ['0', '4'] else 'Check Out'
            })
