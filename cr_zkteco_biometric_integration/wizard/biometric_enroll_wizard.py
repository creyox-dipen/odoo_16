...
class BiometricEnrollWizard(models.TransientModel):
    ...
    device_id = fields.Many2one('biometric.device', string='Biometric Device', required=True, domain=[('active', '=', True)])
    enroll_type = fields.Selection([
        ('finger', 'Fingerprint'),
        ('face', 'Face')
    ], string='Enrollment Type', default='finger', required=True)
    ...
    finger_index = fields.Selection([
        ...
        ('8', 'Right Ring'),
        ('9', 'Right Little'),
    ], string='Finger', default='5', help="Select which finger to enroll.")
        ...
        if command_text:
            ...
            return {
                ...
                'params': {
                    ...
                    'message': _('The machine has been notified. Please place the employee\'s %s on the device.') % 
