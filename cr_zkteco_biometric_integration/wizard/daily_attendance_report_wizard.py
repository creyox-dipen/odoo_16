...
class BiometricDailyAttendanceReportWizard(models.TransientModel):
    ...
    def action_print_pdf(self):
        return self.env.ref('cr_zkteco_biometric_integration.action_report_daily_attendance').report_action(self)

    def get_report_data(self):
        user_tz = pytz.timezone(self.env.user.tz or 'Asia/Kolkata')
        
        # Start and End of the selected day in UTC
        start_utc = user_tz.localize(datetime.combine(self.date, time.min)).astimezone(pytz.utc).replace(tzinfo=None)
        end_utc = user_tz.localize(datetime.combine(self.date, time.max)).astimezone(pytz.utc).replace(tzinfo=None)
        ...
        for att in attendances:
            ...
            out_local = pytz.utc.localize(att.check_out).astimezone(user_tz) if att.check_out else None
