...
class BiometricAbsenceReportWizard(models.TransientModel):
    ...
    def action_print_pdf(self):
        return self.env.ref('cr_zkteco_biometric_integration.action_report_absence').report_action(self)
    ...
    def get_report_data(self):
        ...
        for i in range(delta.days + 1):
            ...
            start_utc = user_tz.localize(datetime.combine(current_date, time.min)).astimezone(pytz.utc).replace(tzinfo=None)
            end_utc = user_tz.localize(datetime.combine(current_date, time.max)).astimezone(pytz.utc).replace(tzinfo=None)
