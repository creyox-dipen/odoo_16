    ...
    def action_calculate(self):
        ...
        if not logs:
            return {
                ...
                'params': {
                    ...
                    'message': _('No new logs found to process for the selected range.'),
                    'sticky': False,
                }
            }
        ...
        for emp_id, dates in grouped_logs.items():
            for att_date, day_logs in dates.items():
                ...
                start_local = local_tz.localize(datetime.combine(att_date, time.min)).astimezone(pytz.utc).replace(tzinfo=None)
                end_local = local_tz.localize(datetime.combine(att_date, time.max)).astimezone(pytz.utc).replace(tzinfo=None)
                ...
                if existing_atts:
                    ...
                    for other in other_atts:
                        if other.check_in: all_outs.append(other.check_in)
                        if other.check_out: all_outs.append(other.check_out)
                    
                    final_out = max([o for o in all_outs if o]) if any(o for o in all_outs if o) else None
