...
class CalDAVAccount(models.Model):
    ...
    def _onchange_server_type(self):
        ...
        urls = {
            ...
            "other": "https://caldav.icloud.com/<YOUR_ACCOUNT_ID>/calendars/<CALENDAR_UUID>/",
    ...
    def _compute_google_auth_status(self):
        ...
        for rec in self:
            if rec.server_type != "google":
                ...
                rec.google_access_token
                and rec.google_access_token_expiry
                and rec.google_access_token_expiry > fields.Datetime.now()
            ):
                rec.google_auth_status = "✓ Authorized (Token Valid)"
            else:
                rec.google_auth_status = (
                    "⚠ Authorized (Token will refresh on next sync)"
                )
    ...
    def _refresh_google_token(self):
        ...
        try:
            ...
            raise UserError(
                _("Failed to refresh Google token. Please re-authorize the account.")
            )
        except Exception as e:
            _logger.error("Google token refresh error for %s: %s", self.name, e)
            raise UserError(
                _("Failed to refresh Google authentication token: %s", str(e))
            )
        ...
        _logger.info(
            'Google access token refreshed for account "%s", expires %s.',
            self.name,
            expiry,
        )
    ...
    def _do_request(
        self, url, method, body=None, extra_headers=None, expected_codes=None
    ):
        ...
        try:
            ...
            raise UserError(
                _(
                    "CalDAV server returned HTTP %(code)s for %(method)s %(url)s:\n%(body)s",
                    code=e.code,
                    method=method,
                    url=url,
                    body=body_text,
            ...
            raise UserError(
                _(
                    "Cannot connect to CalDAV server at %(url)s:\n%(reason)s",
                    url=url,
                    reason=e.reason,
                )
            )
    ...
    def _propfind(self, url, depth="0", body=None):
        ...
        self.ensure_one()
        headers = {"Depth": depth}
        _, _, data = self._do_request(url, "PROPFIND", body=body, extra_headers=headers)
    ...
    def _fetch_ical_with_etag(self, href):
        ...
        _, headers, data = self._do_request(
            url, "GET", extra_headers={"Accept": "text/calendar"}
        )
        ...
        if self.server_type == "zoho":
            ...
            if not etag:
                ...
                _logger.warning(
                    "[ZOHO] ETag empty in standard headers for %s. Headers: %s",
                    href,
                    headers,
                )
        ...
        try:
            ...
            if self.server_type == "google" and is_etag_conflict and etag:
                ...
                try:
                    return _attempt(
                        None
                    )  # None = no If-Match header = unconditional PUT
                except Exception as retry_exc:
                    _logger.error(
                        "[GOOGLE] Unconditional retry PUT %s also failed: %s",
                        href,
                        retry_exc,
                    )
                    raise UserError(
            ...
            if self.server_type == "zoho" and is_etag_conflict and etag:
                ...
                try:
                    ...
                    _logger.info(
                        "[ZOHO] Retrying PUT %s with fresh ETag=%r", href, fresh_etag
                    )
    ...
    def _delete_event(self, href, etag=None):
        ...
        def _attempt(attempt_etag):
            ...
            if attempt_etag:
                ...
                extra["If-Match"] = quoted_etag
            self._do_request(
                url, "DELETE", extra_headers=extra, expected_codes=[200, 204, 404]
            )
        ...
        try:
            ...
            if (
                self.server_type == "zoho"
                and ("409" in error_str or "412" in error_str)
                and etag
            ):
                _logger.warning(
                    "DELETE %s returned conflict. Fetching fresh ETag and retrying.",
                    href,
                )
                try:
                    fresh_etag, _ = self._fetch_ical_with_etag(href)
      ...
      <C:comp-filter name="VEVENT"/>
        ...
        for response in root.findall("{DAV:}response"):
            ...
            if href_el is not None and href_el.text:
                ...
                etag = (
                    etag_el.text.strip().strip('"')
                    if etag_el is not None and etag_el.text
                    else ""
                )
                etags[href] = etag
        _logger.debug(
            "Found %s events on CalDAV server for account %s: %s",
            len(etags),
            self.name,
            list(etags.keys()),
        )
...
<D:propfind xmlns:D="DAV:">
  <D:prop>
    ...
    def action_view_scheduled_action(self):
        ...
        """
        cron = self.env.ref(
            "cr_odoo_caldav_sync.ir_cron_caldav_sync", raise_if_not_found=False
        )
        if not cron:
