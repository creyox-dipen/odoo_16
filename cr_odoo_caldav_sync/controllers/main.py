    ...
    def google_oauth_callback(self, code=None, state=None, error=None, **kwargs):
        ...
        if error:
            ...
            return request.redirect(
                "/web#action=cr_odoo_caldav_sync.action_caldav_account"
            )
        ...
        if not account.exists() or account.server_type != "google":
            _logger.error(
                "Google OAuth callback: account id=%s not found or not Google type.",
                account_id,
            )
            return request.redirect(
                "/web#action=cr_odoo_caldav_sync.action_caldav_account"
        ...
        try:
            ...
            _logger.error(
                "Google token exchange failed for account %s: %s", account.name, body
            )
            return request.redirect(
                "/web#action=cr_odoo_caldav_sync.action_caldav_account"
            )
            ...
            _logger.error(
                "Google token exchange error for account %s: %s", account.name, e
            )
            return request.redirect(
                "/web#action=cr_odoo_caldav_sync.action_caldav_account"
            )
        ...
        write_vals = {
            ...
            "google_access_token_expiry": fields.Datetime.now()
        ...
        try:
            ...
            _logger.warning(
                "Failed to fetch userinfo from Google for account %s: %s",
                account.name,
                e,
            )

        target_email = (
            google_email or account.user_id.email or account.user_id.login or ""
        )
        if target_email:
            ...
            if account.url != correct_url:
                ...
                _logger.info(
                    'Auto-set Google CalDAV URL to %s for account "%s".',
                    correct_url,
                    account.name,
                )
