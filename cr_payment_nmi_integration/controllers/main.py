...
class NmiController(http.Controller):
    ...
    def nmi_ach_process(self, **data):
        ...
        # Clean up any pre-existing card surcharge line for ACH transactions
        for order in tx_sudo.sale_order_ids:
            self._update_order_surcharge(order, provider, "unknown")

        clean_amount = tx_sudo.sale_order_ids[0].amount_total if tx_sudo.sale_order_ids else tx_sudo.amount
        tx_sudo.write({"amount": clean_amount})
